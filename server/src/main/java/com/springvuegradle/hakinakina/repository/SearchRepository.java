...
public interface SearchRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
    ...
    @Query(value = "SELECT u.user_id, u.first_name, u.middle_name, u.last_name FROM User u INNER JOIN User_Activities_Shared r ON u.user_id = r.user_id WHERE r.activity_id = :activityId", nativeQuery = true)
    ...
    @Query(value = "SELECT u.user_id, u.first_name, u.middle_name, u.last_name FROM User u INNER JOIN User_Activity_Role r ON u.user_id = r.user_id WHERE r.activity_id = :activityId AND activityRole = 0", nativeQuery = true)
